...
;примером использования сопроцессора и функций в DLL-библиотеках
.486
...
include	prg_3.inc	;включаемый файл с определением имен идент. меню и диалога
...
;объявление оконной и диалоговых функций объектами, видимыми за пределами данного кода 
public	WindowProc
public	DialogProc1
public	DialogProc2
.data
...
y_P	dd	140	;начальная y-координата центра окружности
x_P	dd	200	;начальная x-координата центра окружности
...
uses ebx,edi,esi,ecx	;эти регистры обязательно должны сохраняться
...
;генерация сообщения WM_PAINT для вывода строки на экран
	...
	jmp	@@exit
@@idmdlllaces_1:
;рисуем узор из окружностей. рекурсивная функция для рисования находится в DLL-библиотеке:
;DrawPattern_1(hwnd,hdc,x,y,r,p) - функция не работает с локальными переменными;
;для начала очистим окно
;выполним первичное заполнение растра серым цветом
;получим дескриптор серой кисти hbrush=GetStockObject(GRAY_BRUSH)
	push	GRAY_BRUSH
	call	GetStockObject
...
@@idmdlllaces_2:
;рисуем узор из окружностей. рекурсивная функция для рисования находится в DLL-библиотеке:
;DrawPattern_2(hwnd,hdc,x,y,r,p) - функция работает с локальными переменными;
;для начала очистим окно
;выполним первичное заполнение растра серым цветом
;получим дескриптор серой кисти hbrush=GetStockObject(GRAY_BRUSH)
	push	GRAY_BRUSH
...
@@wminitdialog:
	jmp	@@exit_true
@@idok:
;прочитаем Xstart UINT GetDlgItemText(  HWND hDlg,       // handle of dialog box
