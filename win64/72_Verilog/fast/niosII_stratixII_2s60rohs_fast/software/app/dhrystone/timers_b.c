...
extern int alt_timestamp_start();
extern int alt_timestamp();
extern int alt_timestamp_freq();
...
clock_t dhry_ctime()
{
  ...
  if (!started)
  {
    ...
    {
          /* failed to start, print a message explaining why */
      printf("Error calling alt_timestamp_start().  Exiting.");
      exit(0);
    }
  }
...
double convert_to_seconds (clock_t c)
{
  double q;
  q = (double)c / (double)alt_timestamp_freq ();
  return q;
}
...
double dtime()
{
  return convert_to_seconds(dhry_ctime());
}
