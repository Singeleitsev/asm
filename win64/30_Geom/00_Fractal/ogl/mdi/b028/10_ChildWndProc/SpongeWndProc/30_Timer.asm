...
GetX:
;xLeft = rand () % lpSpongeData->cxClient ;
	mov rax,lpSpongeData
	movsx rcx,word ptr [rax] ;lpSpongeData->cxClient
	rdrand rax
	xor rdx,rdx
	div ecx
	mov r8,rdx ;We need Remainder only
	and r8,0FFFFh
;xRight = rand () % lpSpongeData->cxClient ;
	mov rax,lpSpongeData
	movsx rcx,word ptr [rax] ;lpSpongeData->cxClient
	rdrand rax
	xor rdx,rdx
...
GetY:
;yTop = rand () % lpSpongeData->cyClient ;
	mov rax,lpSpongeData
	movsx rcx, word ptr [rax+8] ;lpSpongeData->cyClient
	rdrand rax
	xor rdx,rdx
	div ecx
	mov r8,rdx ;We need Remainder only
	and r8, 0FFFFh
;yBottom = rand () % lpSpongeData->cyClient ;
	mov rax,lpSpongeData
	movsx rcx, word ptr [rax+8] ;lpSpongeData->cyClient
	rdrand rax
	xor rdx,rdx
