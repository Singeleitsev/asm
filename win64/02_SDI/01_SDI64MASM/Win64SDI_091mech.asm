...
;============================================================================
WinMain proc
;This Procedure doesn't have parameters hence Assembler will NOT align stack
;Align Stack manually by 8 Bytes for Return Address
;and reserve 80h Bytes as Buffer forpossible 16 Parameters in WinMain
	sub rsp,88h
...
mov rax,rsp
Spell64rax 0,szAppTitle,szStackPointer
;There is no need to Restore the Stack before the exit
	xor rcx,rcx
	call ExitProcess
WinMain endp
...
;This Procedure has Parameters hence Assembler WILL add following instructions here:
	;push rbp ;<-- rsp-8
	;mov rbp,rsp ;<--
;Stack usage: 8(Return Address), 8(push), 20h(Parameters) = 30h
;Hence Stack alignment by 8 Bytes is NOT needed in this Procedure
;Just reserve 80h Bytes as Buffer for possible 16 Parameters in ChildProc
	sub rsp,80h
...
defWndProc:
	...
	call DefWindowProc
finWndProc:
;Assembler will add the 'leave' instruction here
;Stack will be restored automatically
	ret
WndProc endp
end