...
xor rbx,rbx
mov index,ebx
mov rsi,gpSwapchainImageViews ;<-- We don't use it anymore!
mov pViews,rsi
mov rdi,gpFramebuffers
mov pBuffers,rdi

;Build the 2-element attachments array: Color + Depth
create_fb:
mov rsi,pViews
mov rax, qword ptr[rsi] ;swapchain image view handle
mov attachmentColor,rax ;attachment[0] = color
mov rax,ghVkDepthImageView ;depth view (same for all framebuffers)
mov attachmentDepth,rax ;attachment[1] = depth

lea rax,attachmentColor ;address of attachment[0]
mov frameBufferInfo_pAttachments,rax ;<-- Why not gpSwapchainImageViews[index]?!

LOG_TEXT szVkCreateFrameBuffer
...
destroy_fb:
LOG_TEXT szVkDestroyFrameBuffer
mov rcx,ghVkLogicalDevice
mov rsi,pBuffers
