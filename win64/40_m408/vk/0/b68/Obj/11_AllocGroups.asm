;-----------------------------------------------------
;allocGroups – Allocates memory for unique group names
;Input: gnTotalGroups (from countEntities)
;Output: gnUniqueGroups,gpDataGroups allocated and filled
;---------------------------------------------------------
allocGroups proc
...
mov gnUniqueGroups,1

;3.1. Pass 1 - Parse OBJ and collect Unique Names in the Name Array
lbl_Pass_1:

;Allocate temporary buffer (gnTotalGroups + 1) * NAME_BUFFER_SIZE
LOG_TEXT szLogMemoryAllocating
mov rcx,hHeap
mov rdx,8 ;HEAP_ZERO_MEMORY
xor r8,r8
mov r8d,gnTotalGroups
inc r8d
...
;Check for "g" token
cmp word ptr [rcx],2067h ;"g" with Space in reverse order
je lbl_TokenGroup
cmp word ptr [rcx],0967h ;"g" with Tab in reverse order
je lbl_TokenGroup
jmp lbl_SkipByte

lbl_TokenGroup:
;Copy name to temp buffer
add pCurrentSource,2 ;skip "g "
mov rcx,pCurrentSource
...
inc gnUniqueGroups
mov r8d,gnUniqueGroups
cmp r8d,gnTotalGroups
jge lbl_Pass_2 ;Done
...
;3.2. Pass 2 - Allocate and fill Groups Metadata Array
lbl_Pass_2:

;Allocate Memory for Groups Metadata Array
LOG_TEXT szLogMemoryAllocating
mov rcx,hHeap
mov rdx,8 ;HEAP_ZERO_MEMORY
xor r8,r8
mov r8d,gnUniqueGroups
imul r8,r8,GROUP_METADATA_SIZE
call HeapAlloc
test rax,rax
jz lbl_WinError
mov gpDataGroups,rax
LOG_TEXT szOK
...
lbl_NextName:
;Fill GROUP_NAME fields of the Group Metadata Array
xor rcx,rcx
mov ecx,index
...
xor rdx,rdx
mov edx,index
imul rdx,rdx,GROUP_METADATA_SIZE
add rdx,gpDataGroups
add rdx,OFFSET_GROUP_NAME ;Destination = group name field
call parseString
...
LOG_TEXT szLogUniqueGroups
xor rcx,rcx
mov ecx,gnUniqueGroups
