...
;Handles
	hwnd dq 0
	hInst dq 0
	memdc dq 0 ;!!!это глобальная переменная ;<----- new
	maxX dq 0 ;!!!это глобальная переменная ;<----- new
	maxY dq 0 ;!!!это глобальная переменная ;<----- new
...
WindowProc proc uses rbx rdi rsi, @@hwnd:QWORD, @@mes:QWORD, @@wparam:QWORD, @@lparam:QWORD
	...
	local @@hdc:QWORD, @@hbrush:QWORD, @@hbit:QWORD ;<----- new
...
wmcreate:
;создание растрового изображения, совместимого с окном приложения ;<----- new
...
;получить контекст устройства окна на экране
	...
	call GetDC
	mov @@hdc,rax
;получить совместимый контекст устройства памяти  ;<----- new
	mov rcx,@@hdc
	call CreateCompatibleDC
	mov memdc,rax ;!!! memdc - глобальная переменная
;получить дескриптор растрового изображения в памяти ;<----- new
	mov rcx,@@hdc
	mov rdx,maxX
	mov r8,maxY
	call CreateCompatibleBitmap
	mov @@hbit,rax
;выбираем растр в контекст памяти SelectObject(memdc,@@hbit) ;<----- new
	mov rcx,memdc
	mov rdx,@@hbit
	call SelectObject
;выполним первичное заполнение растра серым цветом ;<----- new
;получим дескриптор серой кисти
	mov rcx,2 ;GRAY_BRUSH
	call GetStockObject
	mov @@hbrush,rax
;выбираем кисть в контекст памяти SelectObject(memdc,@@hbrush) ;<----- new
	mov rcx,memdc
	mov rdx,@@hbrush
	call SelectObject
;заполняем выбранной кистью виртуальное окно ;<----- new
...
;выведем строку текста в окно
	...
	call DrawTextA
;генерация сообщения WM_PAINT для вывода строки на экран ;<----- new
	mov rcx,@@hwnd
	xor rdx,rdx
	mov r8,1
	call InvalidateRect
